...
library(wesanderson)
 ggplot(data=d5_na,aes(y=(ratio_activity),x=d1_treat))+ 
   geom_boxplot(aes(fill=d5_treat))+
   ggtitle("Metabolic Poise on Day 5")+ #change enzyme name
   xlab("Mother Treatment")+
   ylab("Ratio Activity (IU/gfw)")+ #change enzyme name+
   scale_x_discrete(labels=c("con" = "Control", "hyp" = "Hypoxia"))+
   scale_fill_manual(values=c("#CCFFCC","#99CCFF"),name="Legend",labels=c("Day 5 Control", "Day 5 Hypoxia"))+
   theme_bw()
...
ggplot(data=d1,aes(y=(ldh_activity),x=mother))+
  ...
  ggtitle("LDH Activity on Day 1 by Mother and Treatment")+
  xlab("Rockfish Mother")+
  ylab("LDH Activity (IU/gfw)")+
  scale_x_discrete(labels=c("G72" = "Mother 1", "G73" = "Mother 2", "G79" = "Mother 3", "G91" = "Mother 4"))+
  scale_fill_manual(values=c("#CCFFCC","#99CCFF"),name="Legend",labels=c("Control", "Hypoxia"))+
  theme_bw()
