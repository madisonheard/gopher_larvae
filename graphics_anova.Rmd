...
```{r}
library(ggplot2)
ggplot(data=d1,aes(y=(ldh_activity),x=mother))+
  geom_boxplot(aes())+
  ggtitle("LDH Activity on Day 5 by Treatment")+ #change enzyme name
  xlab("Treatment")+
  ylab("LDH Activity (IU/gfw)")+ #change enzyme name
  theme_bw()
...
t.test(con$ratio_activity, hyp$ratio_activity)
...
mod<-aov(ratio_activity~d5_treat*d1_treat,data=d5_na)
summary(mod)
...
summary(mod)
interaction.plot(d5_na$d1_treat, d5_na$d5_treat, d5_na$diff)
