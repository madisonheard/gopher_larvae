...
library(wesanderson)
ggplot(data=d5_na,aes(y=(ldh_activity),x=d1_treat))+
  geom_boxplot(aes(fill=d5_treat))+
  ggtitle("Anaerobic Activity on Day 5")+ #change enzyme name
  xlab("Day 5 Treatment")+
  ylab("Anaerobic Activity")+ #change enzyme name+
  scale_x_discrete(labels=c("con" = "Control", "hyp" = "Hypoxia"))+
  scale_fill_manual(values=c("#99CCFF", "#FF9999"))+
  theme_bw()
ggsave("ldh_d5_d1treat.pdf",width=8, height=5) #change enzyme name
```
...
test<-aov(data=d5_na,(ldh_activity)~factor(d5_treat)*factor(d1_treat))
summary(test)

test<-aov(data=d5_na,sqrt(cs_activity)~factor(d1_treat)*factor(mother))
summary(test)
