...
```{r}
boxplot(cs_newdata$cs_activity ~ cs_newdata$d1_treat*cs_newdata$mother, horizontal = TRUE, main="CS Activity at Day 1 separated by treatment", xlab = "CS Activity", ylab = "Treatment")
```



#Using Base R - Activity at Day 1 separated by treatment
```{r}
...
```{r}
library(ggplot2)
ggplot(data=cs_newdata,aes(y=cs_activity,x=d1_treat))+
  geom_boxplot(aes(color=mother))+
  ggtitle("CS Activity on day 1 by mother and treatment")+
  xlab("Treatment")+
  ylab("CS Activity")+
  theme_bw()
ggsave("cs_d1_treat_mother.pdf",width=8, height=5)
```
...
```{r}
library(ggplot2)
ggplot(data=cs_newdata,aes(y=cs_activity,x=d5_treat))+
  geom_boxplot(aes(color=mother))+
  ggtitle("CS Activity on day 5 by mother and treatment")+
  xlab("Treatment")+
  ylab("CS Activity")+
  theme_bw()
```
